...
function combineAndReverseArrays(arr1, arr2) {
  if (Array.isArray(arr1) && Array.isArray(arr2)) {
    const CombidenAndReversed = arr1.concat(arr2).reverse();
    return CombidenAndReversed;
  } else {
    return [];
  }
...
function copyAndSwapElements(arr, target, start, end) {
  if (start < 0 || start >= arr.length || end < 0 || end >= arr.length || target < 0 || target >= arr.length) {
...
function customUnshift(arr, ...elements) {
  if (!Array.isArray(arr)) {
    console.log(Error("Перший аргумент має бути масивом."));
  }
